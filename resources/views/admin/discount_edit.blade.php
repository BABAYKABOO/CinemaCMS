...
@section('content')
    ...
    <div style="text-align: left; margin-left: 20px">
        <form action="{{route('admin-discount-save', $discount->discount_id )}}" enctype="multipart/form-data" method="post">
            @csrf
            <div class="row">
                <div class="col-8" style="text-align: right;">
                    <label class="switch">
                        <input type="checkbox" name="status" {{$discount->status == 1 ? 'checked' : ''}}>
                        <span class="slider round"></span>
                    </label>
                </div>
                <div class="col-3" style="text-align: right;">
                    <label class="form-label">Дата публикации</label>
                    <input type="date"  class="form-control" id="data" name="date" value="{{$discount->date}}" required>
                </div>
            </div>
            <div class="mb-3">
                <label class="form-label">Название акции</label>
                <input type="text" class="form-control" id="name" placeholder="Название" name="name" value="{{$discount->name}}" required>
            </div>
            <div class="mb-3">
                <label class="form-label">Описание</label>
                <textarea class="form-control" aria-label="With textarea" name="desc" placeholder="Описание" id="desc" required>{{$discount->desc}}</textarea>
            </div>
            <div class="mb-3" style="width: 200px;">
                <label for="icon_upload">Главная:<br>
                    <div class="icon_wrapper" style="height: 150px; width: 200px;">
                        <div id="preview_1" style="background: url({{$discount->image_url}}); height: 150px; width: 200px; background-size: 100%">

                        </div>
                    </div>
                </label>
                <input type="file" name="mainimg" preview-target-id="preview_1" title="1">
                <style>
                    .icon_wrapper {
            ...
            <div class="mb-3" style="width: 200px;">
                <label for="icon_upload">Вверхний баннер:<br>
                    <div class="icon_wrapper" style="height: 100px; width: 200px;">
                        <div id="top-preview_1" style="background: url({{$topbanner->image_url}}); height: 100px; width: 200px; background-size: 100%">

                        </div>
                    </div>
                </label>
                <input type="file" name="topbanner" preview-target-id="top-preview_1" title="1">
                <style>
                    .icon_wrapper {
                        height: 90px; width: 200px;
                        background-color: #fefefe;
